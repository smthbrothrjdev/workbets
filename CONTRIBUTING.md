...
1. Add `VITE_CONVEX_URL` as a CI secret.
2. Export it during the build step so Vite can inline it:
   ```bash
   export VITE_CONVEX_URL="$VITE_CONVEX_URL"
   npm run build
   ```
3. For Fly Docker builds, pass it as a build arg from the CI secret:
   ```bash
