				...
				<nav className="hidden items-center gap-2 overflow-x-auto text-xs font-semibold text-slate-600 sm:text-sm md:flex md:gap-6">
					{navigation.map((item) => (
						<button
							...
							className={`whitespace-nowrap rounded-full px-3 py-1.5 transition ${activeTab === item.key
							...
							<div className="flex flex-col gap-1">
								{navigation.map((item) => (
									<button
										...
										type="button"
										onClick={() => handleNavClick(item.key)}
										className={`rounded-2xl px-3 py-2 text-left text-sm font-semibold transition ${activeTab === item.key
												? "bg-slate-900 text-white"
								...
								{action?.includeInMobileMenu && (
									<button
										type="button"
										className="mt-2 rounded-2xl border border-slate-200 bg-red-200 px-3 py-2 text-left text-sm font-semibold text-slate-700"
										onClick={() => {
											action.onClick?.();
											setIsMobileMenuOpen(false);
										}}
